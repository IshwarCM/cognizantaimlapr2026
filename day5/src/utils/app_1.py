...
def display_customers(customer_store):
    ...
    if env == "Production":
        ...
        # Build absolute path to the JSON file using project_root
        file_path = os.path.join(project_root, config.resource_path)

        data_loader.load_data(file_path, customer_store)
